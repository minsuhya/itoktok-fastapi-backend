...
-- 테이블 itoktok_db.announcement 구조 내보내기
...
CREATE TABLE IF NOT EXISTS `board` (
  ...
  `id` int(11) NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`),
  KEY `ix_board_title` (`title`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- 내보낼 데이터가 선택되어 있지 않습니다.

-- 테이블 itoktok_db.boardinfo 구조 내보내기
CREATE TABLE IF NOT EXISTS `boardinfo` (
  ...
  `id` int(11) NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`),
  KEY `ix_boardinfo_board_name` (`board_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
-- 테이블 itoktok_db.center_info 구조 내보내기
CREATE TABLE IF NOT EXISTS `center_info` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `username` varchar(20) NOT NULL COMMENT 'Center Username',
  `center_name` varchar(20) NOT NULL COMMENT '센터명',
  `center_summary` varchar(100) NOT NULL COMMENT '센터 한줄소개',
  `center_introducs` varchar(255) NOT NULL COMMENT '센터 소개',
  `center_export` varchar(50) NOT NULL COMMENT '전문분야',
  `center_addr` varchar(255) NOT NULL COMMENT '센터주소',
  `center_tel` varchar(15) NOT NULL COMMENT '센터전화번호',
  `created_at` datetime DEFAULT current_timestamp(),
  `updated_at` datetime DEFAULT NULL,
  `deleted_at` datetime DEFAULT NULL,
  PRIMARY KEY (`id`),
...
-- 테이블 itoktok_db.customer 구조 내보내기
...
CREATE TABLE IF NOT EXISTS `hero` (
  ...
  KEY `team_id` (`team_id`),
  KEY `ix_hero_age` (`age`),
  KEY `ix_hero_name` (`name`),
  CONSTRAINT `hero_ibfk_1` FOREIGN KEY (`team_id`) REFERENCES `team` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- 내보낼 데이터가 선택되어 있지 않습니다.

-- 테이블 itoktok_db.inquiry 구조 내보내기
...
-- 테이블 itoktok_db.schedule_list 구조 내보내기
CREATE TABLE IF NOT EXISTS `schedule_list` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `schedule_id` int(11) NOT NULL,
  `schedule_date` date DEFAULT curdate(),
  `schedule_time` varchar(5) NOT NULL,
  `schedule_status` char(1) NOT NULL DEFAULT '1' COMMENT '스케줄 상태: 1(상담예약), 2(상담완료), 3(예약취소), 4(노쇼)',
  `schedule_memo` text NOT NULL DEFAULT '' COMMENT '상담일지',
  `created_at` timestamp NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `deleted_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
