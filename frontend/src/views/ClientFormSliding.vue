...
import { registerClientInfo, readClientInfo } from '@/api/client'
...
const consultant_options = ref([
  { value: 'field1', text: 'Field 1' },
  { value: 'field2', text: 'Field 2' },
  { value: 'field3', text: 'Field 3' }
])
</script>
    ...
    <div className="w-full bg-neutral-50 shadow-lg rounded-lg p-6">
      <div className="border-b pb-2 mb-4">
        ...
        <Form @submit="onSubmit" :validation-schema="schema" :initial-values="form" class="space-y-4 text-sm">
          ...
          <div>
            <label class="block mb-1 text-gray-700">상담사 <span class="text-red-500">*</span></label>
            <Field name="consultant" v-model="form.consultant" as="select"
              class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2">
              <option value="">상담사를 선택하세요.</option>
              <option v-for="option in consultant_options" :key="option.value" :value="option.value">
                {{ option.text }}
              </option>
              <!-- 상담사 옵션 추가 -->
            </Field>
          ...
          <div>
            ...
            <Field name="tags" as="input" type="text" v-model="form.tags"
              class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2" />
          ...
          <div>
            ...
            <Field name="memo" as="textarea" class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2" rows="3"
              v-model="form.memo" placeholder="내담자의 주소 문제를 메모해보세요. 최대 100자 까지 입력하실 수 있습니다." />
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <Field name="birth_date" as="input" type="date" v-model="form.birth_date"
                class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2" placeholder="2000.01.01" />
              <ErrorMessage name="birth_date" class="text-red-500 text-xs italic mt-2" />
            </div>
            <div>
              <label class="block mb-1 text-gray-700">성별</label>
              <Field v-model="form.gender" name="gender" as="select"
                class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2">
          ...
          <div>
            ...
            <Field name="email_address" as="input" type="email" v-model="form.email_address"
              class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2" />
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-4">
              <Field name="address_region" as="input" v-model="form.address_region"
                class="bg-neutral-50 border border-gray-300 rounded-md p-2" />
              <Field name="address_city" as="input" v-model="form.address_city"
                class="bg-neutral-50 border border-gray-300 rounded-md p-2" />
            </div>
          ...
          <div>
            <label class="block mb-1 text-gray-700">상담 신청 경로 <span class="text-red-500">*</span></label>
            <Field name="consultation_path" as="select" v-model="form.consultation_path"
              class="w-full bg-neutral-50 border border-gray-300 rounded-md p-2">
