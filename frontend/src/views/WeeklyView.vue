...
<template>
  ...
  <div class="flex justify-between items-center mb-4">
    ...
    <div class="flex space-x-2 text-sm font-semibold">
      <button @click="fetchSchedule(today.getFullYear(), today.getMonth() + 1, today.getDate())"
  ...
  <div class="not-prose relative bg-slate-50 rounded-xl overflow-hidden w-full mx-auto dark:bg-slate-800/25">
    <div class="relative rounded-xl overflow-auto">
      <div class="mx-4 my-4 box-content bg-white dark:bg-slate-800 shadow-xl overflow-hidden">
        <div class="grid grid-cols-[70px,repeat(7,1fr)] grid-rows-1 h-auto">
          ...
          <div v-for="(week_days, index) in currentDateInfo.weekDates" :key="index"
            ...
            <span class="sm:inline-block rounded-full bg-slate-400 w-5 h-5 text-white font-semibold">{{
        ...
        <div class="grid grid-cols-[70px,repeat(7,1fr)] grid-rows-1 max-h-full h-full">
          <div class="flex-row h-full">
            <div v-for="times in 11" :key="times"
          ...
          <!-- 요일/날짜 반복 -->
          <div v-for="(day_schedule, dayIndex) in schedule_data" :key="dayIndex"
            ...
            <!-- 시간 반복 -->
            <div v-for="(time_schedules, timeIndex) in day_schedule" :key="timeIndex"
              @click="clickCalendarTime(dayIndex, timeIndex)" class="h-[90px] text-xs text-slate-400 font-medium p-1.5">
              <!-- 시간 대 개별일정들 -->
              <div class="flex-row text-xs text-blue-600 border border-blue-700/40 rounded-md m-1 space-y-1"
                v-for="(time_schedule, itemindex) in time_schedules.slice(0, 2)" :key="itemindex" :class="[
                ...
                <div class="flex justify-between items-center px-1 h-full w-full">
                  ...
                  <span class="ml-auto inline-block">{{ time_schedule.start_time }} ~ {{ time_schedule.finish_time
                    }}</span>
                </div>
                <div class="flex justify-center items-center px-1 h-full w-full">
                  ...
                  <!-- 삭제버튼 -->
                  <button class="text-xs text-blue-600 border border-blue-700/10 rounded-md m-1 p-0.5 bg-blue-400/20"
                    @click="deleteCalendarSchedule(time_schedule.id)">
                    <TrashIcon class="w-4 h-4" />
                  </button>
                </div>
              </div>
              <div v-if="time_schedules.length > 2"
    ...
    <ScheduleFormSliding :isVisible="isVisible" :scheduleId="currentScheduleId" :scheduleListId="currentScheduleListId"
      :scheduleDate="currentScheduleDate" :scheduleTime="currentScheduleTime" @close="toggleForm" class="z-20" />
    <DailyViewSliding :isDailyViewSlidingVisible="isDailyViewSlidingVisible" :scheduleDate="currentScheduleDate"
      :scheduleTime="currentScheduleTime" @close="toggleDailyViewSliding" @clickCalendarSchedule="clickCalendarSchedule"
      class="z-10" />
  </div>
</template>
