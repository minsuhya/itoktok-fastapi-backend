...
// 드롭 핸들러
const handleDrop = (date, time, event) => {
  event.preventDefault()
  if (!draggedSchedule.value) return
  
  dropTargetDate.value = date
  dropTargetTime.value = time
...
const handleScheduleUpdate = async () => {
  try {
    const response = await updateScheduleDateTime({
...
<template>
        ...
        <div class="grid grid-cols-[70px,repeat(7,1fr)] grid-rows-1 max-h-full h-full">
          ...
          >
            ...
            <div
              ...
              @drop="handleDrop(dayIndex, timeIndex, $event)"
              :class="[
  ...
  <div v-if="showUpdateModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-lg w-96">
      ...
      <p class="mb-4">
        {{ dropTargetDate }} {{ dropTargetTime }}시로 일정을 이동하시겠습니까?
      </p>
      <div class="mb-4">
